...
var (
	...
	MinerPovaContract = &cli.StringFlag{
		Name:     "miner.povacontract",
		Usage:    "0x prefixed public address for reputationBased smart contract",
		Category: flags.MinerCategory,
	}
...
// setPovaContract retrieves the reputation contract from the directly specified command line flags.
func setPovaContract(ctx *cli.Context, cfg *ethconfig.Config) {
	if !ctx.IsSet(MinerPovaContract.Name) {
		return
	}
	addr := ctx.String(MinerPovaContract.Name)
	if strings.HasPrefix(addr, "0x") || strings.HasPrefix(addr, "0X") {
		addr = addr[2:]
	}
	b, err := hex.DecodeString(addr)
	if err != nil || len(b) != common.AddressLength {
		Fatalf("-%s: invalid contract address %q", MinerPovaContract.Name, addr)
		return
	}
	cfg.Miner.PovaContract = common.BytesToAddress(b)
}
...
<<<<<<< HEAD
	...
	povaConfig, err := core.LoadPovaConfig(chainDb, gspec)
=======
	config, err := core.LoadChainConfig(chainDb, gspec)
>>>>>>> 604e215d1bb070dff98fb76aa965064c74e3633f
	if err != nil {
		Fatalf("%v", err)
	}
	...
	engine := ethconfig.CreateConsensusEngine(stack, &ethashConfig, cliqueConfig, povaConfig, nil, false, chainDb)
	if gcmode := ctx.String(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	}
