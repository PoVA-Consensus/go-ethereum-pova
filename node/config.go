...
type Config struct {
	...
	// ContractHost is the host interface on which the contract is running. If this
...
// ContractEndpoint resolves an contract address based on the configured host interface
func (c *Config) ContractEndpoint() string {
	if c.ContractHost == "" {
		return ""
	}
	return fmt.Sprintf("%s", c.ContractHost)
}
