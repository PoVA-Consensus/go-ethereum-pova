...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case apitypes.ApplicationPova.Mime:
		// Pova is my Ethereum PoA standard
		povaData, err := fromHex(data)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(povaData, header); err != nil {
			return nil, useEthereumV, err
		}
		// Add space in the extradata to put the signature
		newExtra := make([]byte, len(header.Extra)+65)
		...
		// Get back the rlp data, encoded by us
		sighash, povaRlp, err := povaHeaderHashAndRlp(header)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*apitypes.NameValueType{
			{
				Name:  "Pova header",
				Typ:   "pova",
				Value: fmt.Sprintf("pova header %d [%#x]", header.Number, header.Hash()),
			},
		}
		// Clique uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: povaRlp, Messages: messages, Hash: sighash}
...
// povaHeaderHashAndRlp returns the hash which is used as input for the proof-of-authority
// signing. It is the hash of the entire header apart from the 65 byte signature
// contained at the end of the extra data.
//
// The method requires the extra data to be at least 65 bytes -- the original implementation
// in pova.go panics if this is the case, thus it's been reimplemented here to avoid the panic
// and simply return an error instead
func povaHeaderHashAndRlp(header *types.Header) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("pova header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = pova.PovaRLP(header)
	hash = pova.SealHash(header).Bytes()
	return hash, rlp, err
}
