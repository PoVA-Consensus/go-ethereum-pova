...
// LoadPovaConfig loads the stored pova config if the chain config
// is already present in database, otherwise, return the config in the
// provided genesis specification. Note the returned pova config can
// be nil if we are not in the pova network.
func LoadPovaConfig(db ethdb.Database, genesis *Genesis) (*params.PovaConfig, error) {
	// Load the stored chain config from the database. It can be nil
	// in case the database is empty. Notably, we only care about the
	// chain config corresponds to the canonical chain.
	stored := rawdb.ReadCanonicalHash(db, 0)
	if stored != (common.Hash{}) {
		storedcfg := rawdb.ReadChainConfig(db, stored)
		if storedcfg != nil {
			return storedcfg.Pova, nil
		}
	}
	// Load the pova config from the provided genesis specification.
	if genesis != nil {
		// Reject invalid genesis spec without valid chain config
		if genesis.Config == nil {
			return nil, errGenesisNoConfig
		...
		return genesis.Config.Pova, nil
	}
	// There is no stored chain config and no new config provided,
	// In this case the default chain config(mainnet) will be used,
