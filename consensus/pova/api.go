...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.pova.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.pova.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.pova.lock.RLock()
	defer api.pova.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.pova.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.pova.lock.Lock()
	defer api.pova.lock.Unlock()

	api.pova.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.pova.proposals, address)
}
...
// GetSigner returns the signer for a specific pova block.
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.pova.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.pova.Author(block.Header())
	}
