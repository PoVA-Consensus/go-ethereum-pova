...
// checkpoint creates a Pova checkpoint signer section from the provided list
...
// sign calculates a Pova digital signature for the given block and embeds it
...
// the account may or may not have cast a Pova vote.
...
// Tests that Pova signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestPova(t *testing.T) {
...
func (tt *povaTest) run(t *testing.T) {
	...
	config.Pova = &params.PovaConfig{
		Period: 1,
		Epoch:  tt.epoch,
	}
	genesis.Config = &config

	engine := New(config.Pova, rawdb.NewMemoryDatabase())
	...
	// Pass all the headers through pova and ensure tallying succeeds
